CREATE TABLE 'ds_salaries' ('id' INTEGER,'work_year' INTEGER,'experience_level' TEXT,'employment_type' TEXT,'job_title' TEXT,'salary' INTEGER,'salary_currency' TEXT,'salary_in_usd' INTEGER,'employee_residence' TEXT,'remote_ratio' INTEGER,'company_location' TEXT,'company_size' TEXT);
...
INSERT INTO 'ds_salaries' ('id','work_year','experience_level','employment_type','job_title','salary','salary_currency','salary_in_usd','employee_residence','remote_ratio','company_location','company_size') VALUES 
 ...
 /*
	##General information
   	- Database with 607 registers
 	- Not null information in the database
	...
	#1 Identify the positive points to work in Data Science
		- Remote work for companies located in 33 different countries
		- There are remote vacancies even in Brazil
		- 15 types of work can be done remotely in Data Science with entry level experience
		- Average annual salary for entry-level experience jobs is up to USD 225,000
		- Salaries are attractive in relation to the Brazilian scenario
		        
	#2 Opportunities for growth in the field of Data Science
		- Average salary of up to 450,000 USD
		- It is possible to work remotely on 42 types of jobs in Data Science
        
	#3 Brazilian workers in Data Science
		- Few samples in the base with only 6 records
		- Half of Brazilians work for BR companies. The rest work for US companies
		- No Brazilian works for companies with more than 250 employees
		- No Brazilian has an expert experience level
*/
...
-- Country quantity of the employer's main office or contracting branch
SELECT
...
FROM ds_salaries
WHERE remote_ratio = 100 
	OR company_location = 'BR'
GROUP by company_location
ORDER BY average_salary DESC

-- Number of types of work that are performed full remote
SELECT
...
-- Types of work that are performed full remote and Entry-level experience
SELECT
DISTINCT job_title
FROM ds_salaries
WHERE remote_ratio = 100
...
-- Filter with only employees of Brazilian origin
SELECT
job_title
, salary_in_usd
, experience_level
...
-- Considering the characteristics of the small base of Brazilian employees, we have some updates for filters closer to the current reality
SELECT
